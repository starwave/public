...
class ImageFileManager {
    ...
    func syncServerFiles() {
        ...
        // Change the closure signature to accept the Bool parameter
        fileManager.syncFiles(fileSyncDoneCallback: { syncWasPerformed in
            print("ImageFileManager: file sync is finished")
            if syncWasPerformed {
                print("ImageFileManager: Server file sync was performed, updating cache.")
                self.saveCurrentListCache()
            } else {
                print("ImageFileManager: Server file sync was skipped, not updating cache.")
            }            
