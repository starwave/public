...
class FileSyncManager {
    ...
    func syncFiles(fileSyncDoneCallback: @escaping (Bool) -> Void, onEvent: @escaping (Int, WPath) -> Void) {
        if (WallpaperServiceInfo.getInstance().getIsSyncing()) {
            print("FileSyncManager: files are already syncing or initialzing")
            // fileSyncDoneCallback(false)
            return
        }
        WallpaperServiceInfo.getInstance().setIsSyncing(true)
        _onEvent = onEvent
        ...
        fetchServerFileListVer { versionDate in
            ...
            if (versionDate != wpsi.getlastSyncTime() || !BPUtil.fileExists(sourceRootPath)) {
                print("FileSyncManager: server file sync started")
                DispatchQueue.global(qos: .background).async {
                    self.syncFilesImpl{
                        fileSyncDoneCallback(true)
                        wpsi.setlastSyncTime(versionDate)
                        WallpaperServiceInfo.getInstance().setIsSyncing(false)
                    }
                }
            } else {
                print("FileSyncManager: server file sync isn't needed")
                fileSyncDoneCallback(false)
                WallpaperServiceInfo.getInstance().setIsSyncing(false)
            }
        }
