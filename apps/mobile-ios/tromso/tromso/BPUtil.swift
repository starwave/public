...
class BPUtil {
    ...
    class func getExifDescription(path: String) -> String {
        guard let exifInterface = ExifInterface(path: path) else {
            print("Failed to initialize ExifInterface for path: \(path)")
            return ""
        }
        let imageDescription = exifInterface.getAttribute(attribute: ExifInterface.TAG_IMAGE_DESCRIPTION)
            .trimmingCharacters(in: .whitespacesAndNewlines)
        return imageDescription.isEmpty ? "" : imageDescription
    }
