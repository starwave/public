...
struct TromsoOptionUI: View {
	...
	var body: some View {
			...
			VStack {
				...
				HStack {
					HStack {
						...
						Picker(selection: $_interval.onChange(onIntervalChange), label: Text("Interval")) {
						   ForEach(0 ..< 10) {
							  Text(String($0 + 5))
						   }
						}
						...
						ForEach(_themeLibsList, id: \.self) { themeLib in
							HStack {
								Text("\(themeLib.Label)")
									...
									.background((themeLib.Label == self._selectedThemeLib?.Label) ? self._selectedRowColor : Color(UIColor.clear))
						...
						ForEach(_customWordsList, id: \.self) { customWord in
							HStack {
								...
								Text("\(customWord.Word)")
									...
									.background((customWord.Word == self._selectedCustomWord?.Word) ? self._selectedRowColor : Color(UIColor.clear))
								Spacer()
		...
		.onAppear() {
			...
			self._interval = wpsi.getInterval() - 5
		}
	...
	func onIntervalChange(_ tag:Int) {
		self._parent._serviceConnection?.sendMessageToService(command:MSG.SET_INTERVAL, intOption: self._interval + 5)
	}
